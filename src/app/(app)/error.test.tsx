...
// The boundary now reads its copy via next-intl; render under a fixed `en`
// provider so these assertions stay in English (no per-locale churn).
function renderBoundary(props: { error: Error & { digest?: string }; reset: () => void }) {
  return render(
...
describe("(app) AppErrorBoundary", () => {
  it("renders the polite 'backend temporarily unreachable' headline", () => {
    renderBoundary({ error: fakeError(), reset: vi.fn() });
  ...
  it("surfaces the digest when Next.js provides one — so a screenshot maps to server logs", () => {
    renderBoundary({ error: fakeError({ digest: "486888060" }), reset: vi.fn() });
  ...
  it("omits the digest line when none was attached (synthetic test errors etc.)", () => {
    const { container } = renderBoundary({ error: fakeError(), reset: vi.fn() });
  ...
  it("Retry calls Next.js' reset() so the segment re-renders without a navigation", () => {
    const reset = vi.fn();
    renderBoundary({ error: fakeError(), reset });
  ...
  it("does NOT surface the raw error.message in the UI (could leak internals)", () => {
    renderBoundary({ error: fakeError({ message: "ECONNREFUSED kumbuka-backend:8080" }), reset: vi.fn() });
