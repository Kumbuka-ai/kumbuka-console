...
- **Five screens**, 1:1 with the design prototype in `design/prototype/`:
  ...
  - `/settings` — write-scope policy (`ask | project | global`), who-may-create-scopes (`admins | members`), private-memory locked block. (Connector values live in the connect area's reference on `/overview` — the connector is a public client, PKCE, no secret to rotate.)
